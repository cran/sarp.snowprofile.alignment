...
This vignette demonstrates the workflow of the analysis methods described in detail in *Herla, F., Horton, S., Mair, P., & Haegeli, P. (in review). Snow profile alignment and similarity assessment for aggregating, clustering, and evaluating of snowpack model output for avalanche forecasting.*
```{r setup}
...
relevant for avalanche hazard assessment. The resulting similarity ranges between `[0, 1]`.

Some alignments require the calculation of that similarity score for choosing the best alignment. In that case, the score
can be accessed from the alignment object via `$sim`. Otherwise, the score can be calculated by
...
Therefor, the package provides `medoidSP`, which identifies the medoid profile among a set of profiles, and a wrapper function 
